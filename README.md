...
HikBox Pictures 是一个面向本地照片库的人物图库实验项目。当前代码主要覆盖工作区初始化、source 管理、扫描、人脸检测产物、人物归属，以及后续 WebUI 产品化规格。
...
```bash
hikbox init --workspace /path/to/workspace --external-root /path/to/external-root
```

添加照片 source：

```bash
hikbox source add --workspace /path/to/workspace /path/to/source --label fixture
```

查看 source：

```bash
hikbox source list --workspace /path/to/workspace
```
...
验收目标是验证真实链路：真实 `hikbox` CLI、真实 HTTP 服务、真实浏览器页面交互、真实 SQLite 和真实图片 artifact。截图不作为默认验收依据；只有在视觉或布局不确定、需要人工复核、用户明确要求，或正在排查视觉回归时才保存截图。
