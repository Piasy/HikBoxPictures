...
由 agent 直接生成并入库一套固定真实小图库和 `manifest.json`，并提供一个 `tests/` 下的验收测试，作为扫描、人物归属、WebUI、合并、排除和导出的共同验收基线。与此同时，仓库还需要额外入库一套较小的补充增量扫描图库，供后续 spec 在主基线扫描完成后再次触发增量扫描、验证人脸归属时复用。
...
- 仓库固定包含 `tests/fixtures/people_gallery_scan/manifest.json` 和对应媒体文件；后续 Slice B-G 默认复用这一份 fixture 作为全量基线，不另建缩水图库。
- 仓库额外固定包含 `tests/fixtures/people_gallery_scan_2/manifest.json` 和对应媒体文件，作为“主基线扫描完成后再次触发增量扫描”的补充 fixture；它不替代 `people_gallery_scan/`，只负责提供第二批同人物新增样本。
...
- `people_gallery_scan_2/` 精确包含 15 张支持扫描的新增单目标人物照片，覆盖与主基线相同的 3 个目标人物，每人 5 张；这 15 张照片都必须能作为后续增量扫描输入，用于验证“再次扫描后新增人脸仍归属到既有人物”。
...
- `people_gallery_scan_2/manifest.json` 延续与主基线 `manifest.json` 相同的核心字段形态：至少记录 `people`、`assets`、`expected_person_groups`、`expected_exports`、`tolerances`、`checksums` 和 `provenance`，但内容只描述这批增量样本。
- manifest 可以声明少量 `tolerances`，用于标记真实模型可能不稳定的边界照片或边界 face；3 个目标人物、30 张主基线单目标人物照片、8 张主基线合照，以及 `people_gallery_scan_2/` 的 15 张增量单目标人物照片都不得依赖 tolerance 才能证明核心验收。

### Public Interface

- 固定 fixture 目录：`tests/fixtures/people_gallery_scan/`。
- 补充增量 fixture 目录：`tests/fixtures/people_gallery_scan_2/`。
- manifest 文件：`tests/fixtures/people_gallery_scan/manifest.json`。
- 补充增量 manifest 文件：`tests/fixtures/people_gallery_scan_2/manifest.json`。
...
- AC-8：仓库额外包含 `tests/fixtures/people_gallery_scan_2/manifest.json` 和对应 15 张支持扫描的新增单目标人物照片；3 个目标人物各 5 张，`manifest` 中的 `assets`、`expected_person_groups`、`expected_exports`、`checksums` 和 `provenance` 能独立描述这批增量样本，供后续增量扫描 spec 复用。
...
- `people_gallery_scan_2/` 不新增第二个 Slice 0 主验收入口；它的自动化消费由后续增量扫描/人物归属 spec 定义，但文件、manifest 和 checksum 必须随仓库直接入库，保持自描述且可复用。
...
- 后续 Slice B-G 继续以 `people_gallery_scan/` 作为共同全量基线；需要再次触发增量扫描的后续 spec 可以额外引用 `people_gallery_scan_2/` 作为第二批输入，而不替代主基线 fixture。
