...
本文档描述当前仓库的数据库 schema、migration 机制，以及与 schema 直接相关的运行语义。

## 0. DB Migration 机制

本节是仓库 DB migration 机制的唯一详细约定入口；README 和 AGENTS 只保留指向本文档的说明。
...
- `embedding_v1.sql`
  建立 `embedding.db` 初始基线 schema，包括 `schema_meta`、`face_embeddings` 及索引 `idx_face_embeddings_face_id`。
...
2. 更新 `### 0.3 Migration 变更摘要`，概述该 migration 引入的表、字段、索引或约束变化
3. 更新本文档中对应表的 DDL 描述、当前版本表以及字段语义
4. migration 执行由自动机制完成，无需额外编写调用代码
...
- 扫描运行时显式把 `<repo_root>/.tmp/insightface_model` 作为 InsightFace 模型根目录传入；不依赖 `~/.insightface` 默认目录。
...
- `artifact_files`：当前 session 内产物文件数；等于 `success_faces * 2`，即每个 face 一张 crop 和一张 context。
...
- `worker_pid`：记录主扫描进程在标记 running 时的 PID，用于排查；不保证等于子进程 PID。
...
- artifact 文件名带 session/batch 作用域前缀，并包含 batch item 级唯一标识，不会在同批重复内容照片之间冲突，也不会在重扫时原地覆盖旧文件；只有当新结果成功提交后，旧路径对应文件才会被清理。
- 同一 asset 的重检采用 IoU 复用语义：归一化 bbox IoU `> 0.5` 的新检测框会复用旧 `face_observations.id`，并保留旧 `main` embedding 与既有 assignment；未被新检测框匹配的旧 face 会被删除并清理其 embedding / assignment；只有新增检测框才会写入新的 `face_observations` 与 `face_embeddings`。
...
- `id`：匿名人物 UUID。
- `display_name`：匿名人物为空，WebUI 命名后才会有值。
- `is_named`：`0` 表示匿名人物，`1` 表示已命名人物。
- `status`：`active` 表示仍可见且可继续接收 active assignment 的人物；`inactive` 表示该人物已经退出 active 生命周期，不再出现在首页、详情页或后续可操作人物集合中。会进入 `inactive` 的路径包括：merge 中作为 loser 失效，以及人物详情页批量排除把该人物全部样本排空。
...
- active 且 `is_named=1` 的人物之间，`display_name` 必须按裁剪后的完整字符串精确唯一；不做大小写折叠或别名归并。
- merge 会把 loser 的全部 active assignment 迁移到 winner，并把 loser 标记为 `inactive`；winner 的 `id`、`display_name`、`is_named` 保持不变。
- undo eligibility 不依赖 `updated_at` 模糊推断，而是依赖 `write_revision` 与 merge 账本中的版本快照精确判断“merge 之后是否发生了新的真实人物相关写入”。

### 2.9 `person_name_events`
...
- `algorithm_version`：固定为 `immich_v6_online_v1`。
- `param_snapshot_json`：固定记录 `max_distance=0.5`、`min_faces=3`、`num_results=3`、`embedding_variant='main'`、`distance_metric='cosine_distance'`、`self_match_included=true`、`two_pass_deferred=true`。
...
- `assignment_source`：固定为 `online_v6`。
- `active`：同一张脸同时最多只允许一条 active assignment。
- `evidence_json`：记录匹配到的近邻 face id 列表与距离摘要。

运行时语义：

- 在线归属新增的 active assignment 仍然只写一条 active 记录；merge 不会新建第二条 assignment，而是直接把 loser 现有 active assignment 的 `person_id` 迁移到 winner，并更新 `updated_at`。
...
- 当同一路径图片在后续 `scan start` 中发生重检测失配、处理失败或等价 invalidation，系统会先删除旧的 active assignment / face rows，再按新结果重建；这类删除或失效同样属于真实人物相关写入，会把受影响 `person.write_revision` 递增。

### 2.12 `person_face_exclusions`
...
- `source_assignment_id`：触发这次排除时失效的 `person_face_assignments.id`，用于把详情页选择动作追溯回当时的 active assignment。
- `created_at`：排除真相写入时间，带 `Z` 后缀的 ISO-8601 UTC 时间字符串。

运行时语义：

- 这张表表达的稳定真相是“`face_observation_id` 不能再自动归到 `excluded_person_id`”，而不是“某次按钮点击发生过”。
- 同一对 `face_observation_id + excluded_person_id` 最多只能存在一条记录；重复排除同一人物会被 HTTP 入口拒绝并保持 DB 不变。
...
- 只支持 exactly-two merge，因此每次成功 merge 恰好写入一条 `person_merge_operations`。
...
### 2.14 `person_merge_operation_assignments`
...
- 不写 `flip` embedding，也不承诺其它 variant。
