...
1. 该模块在 v5 里仍是设计目标，代码主流程尚未落地“可训练打分器 + 在线推理”。
2. 现在 `cluster -> person` 归属仍以规则阈值为主（votes / distance / margin / size gate）。
3. 两轮 review 产生的标注可以用于“冷启动训练集”的第一批样本，但还不足以训练稳定泛化模型：
   - 正样本主要集中在你当前关注人物（person 0/1），类别覆盖偏窄；
   - 负样本和“高相似难例”占比不足，容易学到过拟合规则；
   - 需要继续累计多人物、多场景、含冲突边界样本后，再进入正式建模与校准。
...
- person 0：`638 -> 673`（`+35`, `+5.5%`）
- person 1：`464 -> 480`（`+16`, `+3.4%`）

#### 追加实验：仅启用“第 1 层强证据 recall”

目标：在不改主流程结构的前提下，只放开高置信可回收微簇，优先提升 person 0/1 召回。

实验目录（独立缓存副本，便于并行 review）：

- 基线：`.tmp/magface_hdbscan_review_v5_full_20260420`
- 候选：`.tmp/magface_hdbscan_review_v5_full_20260420_layer1_recall_032_s20`
- person 0/1 差异页（基线 -> 候选）：  
  `.tmp/magface_hdbscan_review_v5_full_20260420_layer1_recall_032_s20/review_person_0_1_added_diff_vs_baseline.html`

本轮只调整强证据 recall 参数：

- `person_cluster_recall_distance_threshold: 0.30 -> 0.32`
- `person_cluster_recall_source_max_cluster_size: 3 -> 20`
- 其余参数保持不变（`margin=0.04/top_n=5/min_votes=3/max_rounds=2`）。

结果（基线 -> 候选）：

- `person_cluster_recall_attach_count: 12 -> 24`（`+12`）
- `person_count: 163 -> 152`（小 person 进一步并入头部）
- `cluster_count: 448 -> 448`（不变）
- `noise_count: 687 -> 687`（不变）
- person 0：`673 -> 734`（`+61`, `+9.1%`）
- person 1：`480 -> 480`（不变）

person 0 本轮新增簇：

- `260/267/271/274/283/286/297/308/324/325/341/437`

你重点点名簇在本轮结果：

- 已归入 person 0：`286/341/267`
- 仍未归入 person 1：`180/11/141`

复核结论（你已确认）：

- person 0 新增归属准确率：`100%`
- 该参数档位可作为“强证据召回”的安全基线，后续再叠加第 2 层与结构证据通道（dyad）补 person 1 的长尾召回。
