...
- [ ] **Step 1: 写失败用例（assignment_run 必须记录 algorithm_version 与 param_snapshot_json）**

```python
run = service.start_assignment_run(scan_session_id=1)
assert run.algorithm_version == "v5.2026-04-21"
assert run.param_snapshot_json["preview_max_side"] == 480
```

- [ ] **Step 2: 写失败用例（不允许 `embedding_flip_weight` 参数存在）**
...
- [ ] **Step 3: 封装冻结链路调用（两遍 AHC + person_consensus + recall）并固定 late fusion=max(main,flip)**

```python
similarity = max(sim_main, sim_flip)
```

- [ ] **Step 4: 将 `main/flip` embedding 统一写入 embedding.db 并去除 JSON flip 缓存路径**
...
- [ ] **Step 5: 明确 `noise/low_quality_ignored` 不落 assignment 表**
...
- [ ] **Step 6: 同步 `docs/db_schema.md` 中 assignment_source 与 run 语义描述**
