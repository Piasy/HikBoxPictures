...
- [ ] **Step 1: 写失败用例（param snapshot 必须完整覆盖 spec §7.2 冻结参数）**
...
- [ ] **Step 2: 写失败用例（scan 主链路必须调用 `run_frozen_v5_assignment`）**
...
- [ ] **Step 4: 实现冻结链路核心语义（HDBSCAN、质量门控、低质量微簇回退、person_consensus、最终 AHC、cluster->person recall）并固定 `late fusion = max(main,flip)`，语义对齐 `face_review_pipeline` 对应函数**

```python
similarity = max(sim_main, sim_flip)
```

- [ ] **Step 5: 将 `main/flip` embedding 写入 `embedding.db`，并从 detect 输出构建 executor 输入**
...
- [ ] **Step 6: 接入 `execution_service` 主链路，移除 synthetic observation/hash embedding/filename regex assignment**
...
- [ ] **Step 7: 明确 `noise/low_quality_ignored` 不落 assignment 表，且 assignment_source 仅允许 spec 枚举**
...
- [ ] **Step 9: 增加行为等价测试（同样本下与 `face_review_pipeline` 对比 cluster/person/assignment 统计）**
...
- [ ] **Step 10: 同步 `docs/db_schema.md` 中 assignment_source、assignment_run、param_snapshot 完整字段说明**
...
- [ ] **Step 5: 校验审计功能无需新增 schema 变更（文档统一在 Task 11 收口）**
