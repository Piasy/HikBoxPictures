...
- WebUI 必须提供可用的看图能力：从人脸裁剪可一键查看照片上下文与原图预览，支撑审核、预览与人物管理决策。
...
- 自动聚类、人物建议、人工维护、人物命名。
- 本地 WebUI 与本地 API。
- WebUI 的照片查看与预览能力（人脸裁剪、上下文带框预览、原图预览与快速切换）。
...
- WebUI 看图能力是 P0：审核、预览、人物管理动作前必须能看到照片证据，不能只看分数与文本字段。
...
- 上述四类维护动作都必须在同页完成“人脸裁剪 -> 照片上下文 -> 原图预览”三层查看，无需切到外部图片软件。
...
4. 执行导出时，系统先展示预览统计和样例照片，再写入导出结果。
...
- 人物卡片网格：显示人物封面、姓名、照片数、待审核徽标；支持快速打开该人物最近样本的轻量预览。
...
- 中央区域：该人物的 observation 样本网格，支持多选与批量操作；每个样本默认展示人脸裁剪，并可进入照片上下文预览。
- 右侧检查器：显示当前 observation 的来源照片、人脸质量、相似人物建议、最近归属历史；支持显示当前脸框在原图中的位置。
- 右侧动作区：重命名、设封面、合并、拆分、移出人物、忽略、锁定。
- 顶部统计：人物名称、封面、样本数、照片数、最近更新时间、低置信项数量。
- 快捷操作：支持上一张/下一张、显示或隐藏脸框、打开原图预览层。
...
- 人脸裁剪：用于快速感知该 observation 的清晰度与基本相似性。
- 照片上下文：在原照片比例中展示该脸及附近区域，并高亮当前 observation。
- 原图预览：用于最终确认（多人同框关系、遮挡、误检、近似人物区分）。
...
- 预览区域：显示按当前规则命中的 `only/group` 数量、最近新增资产数，并提供样例照片预览。
...
为保证“可审核、可预览、可管理”，WebUI 首版必须内置统一图片预览器，并在人物详情、待审核、导出预览三类页面复用。预览器最少满足以下要求：

- 三层视图：
  - `crop`：人脸裁剪图，优先用于快速浏览。
  - `context`：原照片中的局部上下文，默认显示当前 observation 脸框。
  - `original`：原图预览，支持缩放与拖拽。
- 统一交互：
  - 单击样本打开 `context`。
  - 双击或快捷键打开 `original`。
  - 支持上一张/下一张切换，避免频繁关闭弹层。
  - 支持脸框显示开关，且当前 observation 必须高亮。
- 场景约束：
  - 人物详情页中的合并、拆分、移出等动作，执行前必须可查看 `original`。
  - 待审核四类队列的“确认/驳回/忽略”动作，执行前必须可查看至少 `context`。
  - 导出模板预览必须能查看命中样例照片，不能只有数量统计。
- 异常降级：
  - 原图不存在、损坏或不可解码时，界面需明确提示原因，并保留该资产路径与失败码。
  - `crop/context` 缺失时允许触发异步重建，不阻断页面继续审核其他项。
...
后端需提供面向 WebUI 的本地图片读取接口，统一处理缩略图、上下文图与原图流式读取。建议接口分层如下：

- `GET /api/observations/{id}/crop`：返回人脸裁剪图。
- `GET /api/observations/{id}/context`：返回带当前 observation 脸框的局部上下文图。
- `GET /api/photos/{id}/preview`：返回受限尺寸的整图预览（用于列表和弹层初次加载）。
- `GET /api/photos/{id}/original`：返回原图流；支持 Range 读取，避免大图一次性读取。

边界约束：

- 只能读取已注册 `library_source` 下的资产路径，禁止任意路径访问。
- 所有文件路径必须做规范化与越界校验，防止路径穿越。
- 对于缺失/损坏文件，返回结构化错误（包含 `photo_asset_id`、错误码、可读提示），供 WebUI 直接展示。
...
- `crop_path` 指向可重建的人脸裁剪 artifact，用于 WebUI 快速展示。
- 当 `crop_path` 对应文件丢失时，不应破坏业务真相；系统可按需重建并回填。
...
WebUI 看图路径额外约束如下（本机环境目标值）：

- `crop/context` 首屏加载 `p95 <= 200ms`（命中本地缓存/缩略图时）。
- 预览层首次打开 `p95 <= 400ms`（常见手机照片分辨率）。
- 原图首字节返回 `p95 <= 600ms`；大图通过分块或 Range 渐进加载。
- 在网格浏览时允许预取相邻 1-2 张，减少逐张审核抖动。
...
- `preview.asset.decode_failed`
- `preview.context.rebuild_requested`
...
1. 在人物详情页点击任一 observation，必须能看到 `context` 视图，且当前 observation 脸框高亮。
2. 在人物详情页执行“合并/拆分/移出人物”前，用户必须可进入 `original` 预览层。
3. 在“可合并”队列中，必须支持并排查看两个人物的样本，并可分别打开对应原图。
4. 在“可拆分”队列中，必须支持查看同一人物下多个 observation 的原图上下文，避免仅看裁剪图误判。
5. 在“低置信归属”队列中，确认或驳回前必须可查看原图上下文，不得仅依赖分数与名称。
6. 在导出模板预览中，除命中数量外，必须至少展示一组命中样例并支持打开预览。
7. 当原图丢失或损坏时，页面需显示可读错误，不得白屏或静默失败。
8. 单张图片加载失败不得阻断队列内其他项的浏览和操作。
9. 快捷键上一张/下一张在人物详情与待审核页行为一致。
10. 预览器在人物详情、待审核、导出预览三处的交互语义保持一致，不出现同操作不同结果。
