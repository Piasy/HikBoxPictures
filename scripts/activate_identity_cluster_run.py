...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="激活 identity cluster run 为 materialization owner")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"identity cluster run 激活失败: {exc}", file=sys.stderr)
        return 1
    finally:
        if orchestrator is not None:
            orchestrator.close()

    print("identity cluster run 激活完成: " + json.dumps(summary, ensure_ascii=False))
    return 0
